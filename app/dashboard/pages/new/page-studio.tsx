...
export default function PageStudio({ userId, userLinks }: { userId: string; userLinks?: any[] }) {
  const {
    plan,
    planName,
    planDisplayName,
  ...
  const [buttonShadow, setButtonShadow] = useState<ButtonShadow>("soft");
  const [buttonTheme, setButtonTheme] = useState<ButtonThemeId>("solid");
  const [buttonTextAlignment, setButtonTextAlignment] = useState<"left" | "center" | "right">("center");
  const [textAlignment, setTextAlignment] = useState<"left" | "center" | "right">("center");
  const [verticalAlign, setVerticalAlign] = useState<"top" | "center">("top");
  ...
  const [bannerStyle, setBannerStyle] = useState<"info" | "success" | "warning" | "error">("info");
  const [bannerPosition, setBannerPosition] = useState<"top" | "bottom">("top");
  const [bannerType, setBannerType] = useState<"text" | "image">("text");
  
  // Layout template
  const [layoutTemplate, setLayoutTemplate] = useState<"centered" | "left" | "card" | "split" | "minimal" | "hero" | "sidebar" | "grid" | "magazine">("centered");
  
  // Social icons customization
  const [socialIconSize, setSocialIconSize] = useState(20);
  const [socialIconStyle, setSocialIconStyle] = useState<"filled" | "outlined" | "minimal">("outlined");
  const [socialIconShape, setSocialIconShape] = useState<"circle" | "square" | "rounded">("circle");
  const [socialIconPadding, setSocialIconPadding] = useState(8);
  const [socialIconGap, setSocialIconGap] = useState(12);
  ...
  const [buttonFontWeight, setButtonFontWeight] = useState(600);
  const [linkGap, setLinkGap] = useState(12);
  const [maxContentWidth, setMaxContentWidth] = useState(400);
    ...
    <>
                ...
                <div className="p-4 rounded-lg bg-gradient-to-r from-electric-sapphire/5 to-bright-indigo/5 border border-electric-sapphire/10">
                  <div className="flex items-center justify-between mb-3">
                    <div>
                      ...
                      <div className="text-xs text-neutral-muted">Add a profile picture</div>
                    </div>
                    <button
                      type="button"
      ...
      {createdPage && (
        <SuccessModal
          ...
          createAnotherText="Create another page →"
          onCreateAnother={() => {
            setShowSuccessModal(false);
